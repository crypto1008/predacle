...
function getTrendLabel(change: number | null | undefined) {
  ...
  return change > 0
    ? { label: `↑${pct}% last month`, color: '#05a66b' }
    : { label: `↓${pct}% last month`, color: '#e5484d' }
}
...
function MarketDetail({ id, initialMarket }: { id: string; initialMarket: Market | null }) {
  ...
  const footBg  = dark ? '#0d0e10' : '#fafbfc'
  const inputBg = dark ? '#26282d' : '#f5f6f8'

  if (loading) return (
    <div style={{ maxWidth: 720, margin: '60px auto', padding: '0 20px', textAlign: 'center' }}>
      <div style={{ width: 40, height: 40, border: `3px solid ${border}`, borderTopColor: '#0052ff', borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 16px' }} />
  ...
  if (notFound) return (
    <div style={{ maxWidth: 720, margin: '80px auto', padding: '0 20px', textAlign: 'center' }}>
      ...
      <button onClick={() => router.push('/markets')}
        style={{ padding: '10px 24px', background: '#0052ff', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
        Browse all markets
      </button>
    </div>
  )
  ...
  const signalStyle = aiSummary ? {
    BULLISH: { color: '#04794e', bg: dark ? '#04291b' : '#e7f8f0', label: '↑ Bullish' },
    BEARISH: { color: '#cf202f', bg: dark ? '#1c0202' : '#fdecec', label: '↓ Bearish' },
    NEUTRAL: { color: '#d97706', bg: dark ? '#1c1002' : '#fffbeb', label: '→ Neutral' },
  }[aiSummary.signal] : null

  // Health badge
  const healthBadge = platformHealth ? {
    live:    { label: '🟢 Live',    color: '#04794e', bg: dark ? '#04291b' : '#e7f8f0' },
    ...
    <main id="main" style={{ maxWidth: 720, margin: '0 auto', padding: '32px 20px 64px', background: bg, minHeight: '80vh' }}>
      ...
      <article style={{ background: cardBg, border: `1px solid ${border}`, borderRadius: 16, overflow: 'hidden', marginBottom: 16, position: 'relative' }}>
        ...
        <div style={{ padding: '24px 24px 20px', borderBottom: `1px solid ${divider}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14, flexWrap: 'wrap' }}>
            ...
            {/* Category */}
            {catLabel && (
              <span style={{ fontSize: 11, color: txt2, background: dark ? '#26282d' : '#f5f6f8', border: `1px solid ${dark ? '#303338' : '#eaecef'}`, padding: '3px 9px', borderRadius: 6, fontWeight: 500 }}>
                {catIcon} {catLabel}
              </span>
            )}
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button onClick={handleCopyLink} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', fontSize: 12, fontWeight: 600, border: `1px solid ${border}`, borderRadius: 8, background: copied ? (dark ? '#04291b' : '#e7f8f0') : cardBg, color: copied ? '#04794e' : txt2, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s' }}>
            ...
            <>
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 16, marginBottom: 12, flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: 1, minWidth: 120 }}>
                  ...
                  <div style={{ height: 8, background: dark ? '#26282d' : '#f5f6f8', borderRadius: 4, overflow: 'hidden' }}>
              ...
              {timelinePct !== null && (
                <div style={{ marginTop: 16 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: 11, color: txt2 }}>
                    <span>Created</span>
                    <span style={{ fontWeight: 600, color: '#0052ff' }}>Today ({timelinePct}% elapsed)</span>
        ...
        <div style={{ padding: '20px 24px', background: footBg, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          ...
          <button onClick={handleTrade} disabled={trading}
            style={{ padding: '12px 28px', fontSize: 14, fontWeight: 700, background: trading ? (dark ? '#26282d' : '#8a919e') : '#0052ff', color: '#fff', border: 'none', borderRadius: 10, cursor: trading ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}>
                ...
                <div style={{ display: 'flex', gap: 4 }}>
                  {(['YES', 'NO'] as const).map(side => (
                    <button key={side} onClick={() => setBetSide(side)}
                      style={{ padding: '10px 20px', fontSize: 13, fontWeight: 700, borderRadius: 8, border: `1px solid ${betSide === side ? (side === 'YES' ? '#05a66b' : '#e5484d') : border}`, background: betSide === side ? (side === 'YES' ? (dark ? '#04291b' : '#e7f8f0') : (dark ? '#1c0202' : '#fdecec')) : cardBg, color: betSide === side ? (side === 'YES' ? '#05a66b' : '#e5484d') : txt2, cursor: 'pointer', fontFamily: 'inherit' }}>
                      {side}
                    </button>
                  ))}
                </div>
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 20, flexWrap: 'wrap' }}>
              {[10, 50, 100, 500, 1000].map(amt => (
                <button key={amt} onClick={() => setBetAmount(amt)}
                  style={{ padding: '4px 10px', fontSize: 12, fontWeight: 600, borderRadius: 6, border: `1px solid ${border}`, background: betAmount === amt ? '#0052ff' : cardBg, color: betAmount === amt ? '#fff' : txt2, cursor: 'pointer', fontFamily: 'inherit' }}>
                  ${amt}
                </button>
              ))}
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, background: dark ? '#0d0e10' : '#f5f6f8', border: `1px solid ${border}`, borderRadius: 12, padding: '16px' }}>
              {[
                { label: 'Profit if correct', val: `$${profit.toFixed(2)}`, color: betSide === 'YES' ? '#05a66b' : '#e5484d' },
                { label: 'Total payout',      val: `$${payout.toFixed(2)}`,  color: txt1 },
                { label: 'Return',            val: `${returnPct}%`,           color: '#0052ff' },
              ].map(r => (
        ...
        <div style={{ padding: '20px 24px' }}>
          {aiLoading && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: txt2 }}>
              <div style={{ width: 16, height: 16, border: `2px solid ${border}`, borderTopColor: '#0052ff', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
          ...
          {aiSummary && (
            <div>
              ...
              <div style={{ background: dark ? '#0d0e10' : '#f5f6f8', border: `1px solid ${border}`, borderRadius: 10, padding: '14px 16px', marginBottom: 12 }}>
            ...
            {crossPlatform.map(cp => {
              ...
              return (
                <a key={cp.id} href={affiliateUrl(cp.platform, cp.url)} target="_blank" rel="noopener noreferrer"
                  ...
                  onMouseEnter={e => e.currentTarget.style.borderColor = '#99b9ff'}
                  onMouseLeave={e => e.currentTarget.style.borderColor = border}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: PLATFORM_COLORS[cp.platform] || '#8a919e', display: 'inline-block' }} />
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    {pct !== null && cpPct !== null && cpPct !== pct && (
                      <span style={{ fontSize: 10, fontWeight: 600, padding: '1px 5px', borderRadius: 4, background: cpPct > pct ? (dark ? '#04291b' : '#e7f8f0') : (dark ? '#1c0202' : '#fdecec'), color: cpPct > pct ? '#05a66b' : '#e5484d' }}>
                        {cpPct > pct ? `+${cpPct - pct}pp` : `${cpPct - pct}pp`}
                      </span>
                    )}
                    <span style={{ fontSize: 11, color: '#0052ff' }}>Trade →</span>
                  </div>
                </a>
              )
            })}
      ...
      {similar.length > 0 && (
        <div style={{ background: cardBg, border: `1px solid ${border}`, borderRadius: 16, overflow: 'hidden', marginBottom: 16 }}>
          <div style={{ padding: '18px 24px', borderBottom: `1px solid ${divider}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <a href={`/markets?category=${market.category}`} style={{ fontSize: 12, color: '#0052ff', textDecoration: 'none', fontWeight: 600 }}>View all →</a>
          </div>
      ...
      <div style={{ textAlign: 'center' }}>
        <button onClick={() => router.back()}
          style={{ fontSize: 13, color: '#0052ff', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 500 }}>
          ← Back to markets
        </button>
      </div>
    </main>
